...
import com.plixplatform.http.api_key
import com.plixplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi
import com.plixplatform.it.sync.config.MatcherPriceAssetConfig
import com.plixplatform.it.util.{GlobalTimer, TimerExt}
import com.plixplatform.it.{Node, api}
...
object AsyncMatcherHttpApi extends Assertions {
  ...
  implicit class MatcherAsyncHttpApi(matcherNode: Node) extends NodeAsyncHttpApi(matcherNode) {
    ...
    def matcherApiEndpoint: URL = new URL(s"http://localhost:${matcherNode.nodeExternalPort(matcherNode.config.getInt("plix.dex.port"))}")
