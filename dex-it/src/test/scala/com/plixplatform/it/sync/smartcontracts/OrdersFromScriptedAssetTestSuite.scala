...
import com.plixplatform.api.http.ApiError.TransactionNotAllowedByAssetScript
...
class OrdersFromScriptedAssetTestSuite extends MatcherSuiteBase {
  ...
  "can match orders when SmartAccTrading is still not activated" in {
    val pair = AssetPair(Plix, IssuedAsset(AllowAsset.id()))
  ...
  "can't execute against unscripted, if the script returns FALSE" in {
    ...
    val setAssetScriptId = node.setAssetScript(AllowAsset2Id, matcher.address, 1.plix, Some(DenyBigAmountScript.bytes().base64)).id
  ...
  "can't execute against scripted, if one script returns FALSE" in {
    ...
    info("update a script")
    val setAssetScriptId = node.setAssetScript(AllowAsset3Id, matcher.address, 1.plix, Some(DenyBigAmountScript.bytes().base64)).id
    node.waitForTransaction(setAssetScriptId)
...
object OrdersFromScriptedAssetTestSuite {
  ...
  private val commonConfig = ConfigFactory.parseString(s"""plix {
