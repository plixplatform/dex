...
import com.plixplatform.it.util.DoubleExt
...
class ExtraFeeTestSuite extends MatcherSuiteBase {
  ...
  {
    val xs = Seq(
      node.broadcastTransfer(alice, bob.address, defaultAssetQuantity / 2, 0.005.plix, Some(asset0), None).id,
      node.broadcastTransfer(alice, bob.address, defaultAssetQuantity / 2, 0.009.plix, Some(asset1), None).id,
      node.broadcastTransfer(bob, alice.address, defaultAssetQuantity / 2, 0.005.plix, Some(asset2), None).id
    )
    ...
    "with one Smart Account and one Smart Asset" - {
      "then fee should be 0.003 + 0.004 (for Smart Asset only, not Smart Account)" in {
        ...
        node.expectRejectedOrderPlacement(
          ...
          invalidFee,
          2,
          expectedMessage = Some("Required 0.007 PLIX as fee for this order, but given 0.00699999 PLIX")
        )
        ...
        node.reservedBalance(alice)("PLIX") shouldBe expectedFee
      ...
      "then fee should be 0.003 + (0.004 * 2) + 0.004 (for Smart Assets and Matcher Script)" - {
        "and total fee should be divided proportionally with partial filling" in {
          ...
          node.expectRejectedOrderPlacement(
            ...
            invalidFee,
            2,
            expectedMessage = Some("Required 0.015 PLIX as fee for this order, but given 0.01499999 PLIX")
          )
