...
class RatesTestSuite extends MatcherSuiteBase {
  ...
  val defaultRateMap: Map[Asset, Double] = Map(Plix -> 1d)
  ...
  "Rates can be handled via REST" in {
    ...
    // update rate for Plix is not allowed
    node.upsertRate(Plix, wctRateUpdated, expectedStatusCode = BadRequest).message shouldBe "Rate for Plix cannot be changed"
    node.getRates shouldBe defaultRateMap + (wctAsset -> wctRateUpdated)
