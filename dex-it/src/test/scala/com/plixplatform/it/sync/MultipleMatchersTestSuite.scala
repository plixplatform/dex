...
class MultipleMatchersTestSuite extends MatcherSuiteBase {
  private def configOverrides = ConfigFactory.parseString("""plix.dex {
      |  price-assets = ["PLIX"]
  ...
  private def matcher2NodeConfig =
    ConfigFactory
      ...
      |}
      |plix.dex.events-queue.kafka.group = 1""".stripMargin)
  ...
  private val assetPair3                   = AssetPair(assetPair1.priceAsset, Plix)
