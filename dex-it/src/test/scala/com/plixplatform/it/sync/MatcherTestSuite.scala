...
import com.plixplatform.it.api.{AssetDecimalsInfo, LevelResponse}
...
class MatcherTestSuite extends MatcherSuiteBase with TableDrivenPropertyChecks {
  ...
  "Check cross ordering between Alice and Bob" - {
    ...
    val order1         = node.prepareOrder(alice, alicePlixPair, SELL, aliceSellAmount, 2000.plix, version = orderVersion, timeToLive = 2.minutes)
    ...
    "get opened trading markets" in {
      ...
      markets.priceAssetName shouldBe "PLIX"
    ...
    "sell order could be placed correctly" - {
      "alice places sell order" in {
        ...
        node.waitOrderStatus(alicePlixPair, order1Response.message.id, "Accepted")

        // Alice check that order is correct
        val orders = node.orderBook(alicePlixPair)
        orders.asks.head.amount shouldBe aliceSellAmount
        orders.asks.head.price shouldBe 2000.plix
      }
      ...
      "and should match with buy order" in {
        ...
        val order2 = node.placeOrder(bob, alicePlixPair, BUY, 200, 2.plix * Order.PriceConstant, matcherFee, orderVersion)
        order2.status shouldBe "OrderAccepted"

        node.waitOrderStatus(alicePlixPair, order1Response.message.id, "PartiallyFilled")
        node.waitOrderStatus(alicePlixPair, order2.message.id, "Filled")

        node.orderHistoryByPair(bob, alicePlixPair).map(_.id) should contain(order2.message.id)
      ...
      "submitting sell orders should check availability of asset" in {
        ...
        val badOrder = node.prepareOrder(bob, alicePlixPair, SELL, 300, 1900.plix, orderVersion)
        ...
        val order3 = node.placeOrder(bob, alicePlixPair, SELL, 150, 1900.plix, matcherFee, orderVersion)
        node.waitOrderStatus(alicePlixPair, order3.message.id, "Accepted")

        // Bob checks that the order in the order book
        val orders = node.orderBook(alicePlixPair)
        orders.asks should contain(LevelResponse(150, 1900.plix))
      }
      ...
      "buy order should match on few price levels" in {
        ...
        val order4 =
          node.placeOrder(alice, alicePlixPair, BUY, 350, (21.plix / 10.0 * Order.PriceConstant).toLong, matcherFee, orderVersion)
        order4.status should be("OrderAccepted")

        // Where were 2 sells that should fulfill placed order
        node.waitOrderStatus(alicePlixPair, order4.message.id, "Filled")
      ...
      "order could be canceled and resubmitted again" in {
        ...
        val status1 = node.cancelOrder(alice, alicePlixPair, order1Response.message.id)
        ...
        val order4 = node.placeOrder(alice, alicePlixPair, SELL, 100, 2000.plix, matcherFee, orderVersion)
        order4.status should be("OrderAccepted")

        // Alice checks that the order is in the order book
        val orders2 = node.orderBook(alicePlixPair)
        orders2.asks should contain(LevelResponse(100, 2000.plix))
      }
      ...
      "buy order should execute all open orders and put remaining in order book" in {
        ...
        val order5 = node.placeOrder(bob, alicePlixPair, BUY, 130, 2000.plix, matcherFee, orderVersion)

        // Check that the order is partially filled
        node.waitOrderStatus(alicePlixPair, order5.message.id, "PartiallyFilled")

        // Check that remaining part of the order is in the order book
        val orders = node.orderBook(alicePlixPair)
        orders.bids should contain(LevelResponse(30, 2000.plix))
      ...
      "request order book for blacklisted pair" in {
        val f = node.matcherGetStatusCode(s"/matcher/orderbook/$ForbiddenAssetId/PLIX", 404)
      ...
      "should consider UTX pool when checking the balance" in {
        ...
        val bobPlixPair = AssetPair(IssuedAsset(ByteStr.decodeBase58(bobAsset).get), Plix)

        def bobOrder = node.prepareOrder(bob, bobPlixPair, SELL, someAssetAmount, 0.005.plix, matcherFee, orderVersion)

        val order6 = node.placeOrder(bobOrder)
        node.waitOrderStatus(bobPlixPair, order6.message.id, "Accepted")

        // Alice wants to buy all Bob's assets for 1 Wave
        val order7 = node.placeOrder(alice, bobPlixPair, BUY, someAssetAmount, 0.005.plix, matcherFee, orderVersion)
        node.waitOrderStatus(bobPlixPair, order7.message.id, "Filled")
      ...
      "trader can buy plix for assets with order without having plix" in {
        val bobBalance = node.accountBalances(bob.address)._1
        ...
        def bobOrder =
          node.prepareOrder(bob, bobPlixPair, SELL, someAssetAmount, 1.plix, matcherFee, orderVersion)

        val order8 = node.placeOrder(bobOrder)
        node.waitOrderStatus(bobPlixPair, order8.message.id, "Accepted")
        node.reservedBalance(bob)

        // Bob moves all plix to Alice
        ...
        // Cleanup
        node.cancelOrder(bob, bobPlixPair, order8.message.id).status should be("OrderCanceled")
      ...
      "market status" in {
        ...
        node.placeOrder(bob, bobPlixPair, SELL, askAmount, ask, matcherFee, orderVersion)

        val resp1 = node.marketStatus(bobPlixPair)
        ...
        node.placeOrder(alice, bobPlixPair, BUY, bidAmount, bid, matcherFee, orderVersion)

        val resp2 = node.marketStatus(bobPlixPair)
