...
class BlacklistedTradingTestSuite extends MatcherSuiteBase with GivenWhenThen {
  ...
  "When blacklists are empty" in {
    ...
    val usdOrder  = node.placeOrder(alice, plixUsdPair, BUY, dec8, dec2, matcherFee).message.id
    val wctOrder  = node.placeOrder(alice, wctPlixPair, BUY, dec2, dec8, matcherFee).message.id
    val ethOrder  = node.placeOrder(alice, ethPlixPair, SELL, dec8, dec8, matcherFee).message.id
    val btcOrder1 = node.placeOrder(bob, plixBtcPair, SELL, dec8, dec8, matcherFee).message.id
    node.waitOrderStatus(wctPlixPair, btcOrder1, "Accepted")
    ...
    node.orderStatusExpectInvalidAssetId(wctOrder, wctPlixPair, WctId.toString, _.message.contains("is blacklisted"))
    node.orderStatusExpectInvalidAssetId(ethOrder, ethPlixPair, EthId.toString, _.message.contains("is blacklisted"))
    node.expectRejectedOrderPlacement(alice, wctPlixPair, BUY, dec2, dec8)
    node.expectRejectedOrderPlacement(alice, ethPlixPair, SELL, dec8, dec8)
    node.expectRejectedOrderPlacement(bob, plixBtcPair, SELL, dec8, dec8)

    And("orders of blacklisted address are still available")
    node.orderStatus(btcOrder1, plixBtcPair).status shouldBe "Accepted"

    And("orders for other assets are still available")
    node.orderStatus(usdOrder, plixUsdPair).status shouldBe "Accepted"

    And("OrderBook for blacklisted assets is not available")
    node.orderBookExpectInvalidAssetId(wctPlixPair, WctId.toString, _.message.contains("is blacklisted"))
    node.orderBookExpectInvalidAssetId(ethPlixPair, EthId.toString, _.message.contains("is blacklisted"))
    node.orderBook(plixBtcPair).asks.size shouldBe 1
    ...
    val btcOrder2 = node.placeOrder(alice, plixBtcPair, SELL, dec8, dec8, matcherFee).message.id
    node.waitOrderStatus(plixBtcPair, btcOrder2, "Accepted")
    ...
    node.orderBook(wctPlixPair).bids.size shouldBe 1
    node.orderBook(ethPlixPair).asks.size shouldBe 1

    And("order statuses are available again")
    node.orderStatus(wctOrder, wctPlixPair).status shouldBe "Accepted"
    node.orderStatus(ethOrder, ethPlixPair).status shouldBe "Accepted"

    And("new orders can be placed")
    val newWctOrder = node.placeOrder(alice, wctPlixPair, BUY, dec2, dec8, matcherFee).message.id
    val newEthOrder = node.placeOrder(alice, ethPlixPair, SELL, dec8, dec8, matcherFee).message.id
    val btcOrder3   = node.placeOrder(bob, plixBtcPair, SELL, dec8, dec8, matcherFee).message.id
    node.waitOrderStatus(wctPlixPair, btcOrder3, "Accepted")
    node.orderStatus(newWctOrder, wctPlixPair).status shouldBe "Accepted"
    node.orderStatus(newEthOrder, ethPlixPair).status shouldBe "Accepted"
  }
