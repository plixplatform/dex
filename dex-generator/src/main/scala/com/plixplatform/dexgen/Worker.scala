...
import cats.Show
import com.plixplatform.account.{AddressOrAlias, KeyPair, PublicKey}
import com.plixplatform.api.http.assets.SignedTransferV1Request
import com.plixplatform.common.state.ByteStr
import com.plixplatform.crypto
import com.plixplatform.dexgen.Worker._
import com.plixplatform.dexgen.utils.{ApiRequests, GenOrderType}
import com.plixplatform.it.api.{MatcherResponse, MatcherStatusResponse, OrderbookHistory, Transaction, UnexpectedStatusCodeException}
...
class Worker(workerSettings: Settings,
  ...
  def transfer(i: Long, sender: KeyPair, assetId: Asset, recipient: KeyPair, halfBalance: Boolean)(
    ...
    to(endpoint).balance(sender.address, assetId).flatMap { balance =>
      ...
      val transferAmount = assetId.fold(halfAmount - 0.001.plix)(_ => halfAmount)
      ...
      TransferTransactionV1.selfSigned(assetId,
        ...
        case Right(txRequest) =>
          log.info(
            s"[$tag] ${assetId.compatId.fold("Plix")(_.toString)} balance of ${sender.address}: $balance, sending $transferAmount to ${recipient.address}")
  ...
  def send(i: Long, orderType: GenOrderType.Value): Future[Any] = {
    ...
    val pair = createAssetPair(
      randomFrom(tradingAssets.dropRight(2).dropRight(tradingAssetsSize / 2)).getOrElse(Plix),
      randomFrom(tradingAssets.dropRight(2).takeRight(tradingAssetsSize / 2 - 1)).getOrElse(Plix)
    )
    ...
    val work = orderType match {
      ...
      case GenOrderType.InvalidAmount =>
        ...
        val pair         = AssetPair(randomFrom(tradingAssets.takeRight(2)).get, Plix)
      ...
      case GenOrderType.FakeSell =>
        ...
        val pair                      = AssetPair(randomFrom(tradingAssets.takeRight(2)).getOrElse(Plix), Plix)
        for {
