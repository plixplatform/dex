...
import com.plixplatform.api.http.assets.{SignedIssueV2Request, SignedMassTransferRequest}
...
import com.plixplatform.dexgen.cli.ScoptImplicits
import com.plixplatform.dexgen.config.FicusImplicits
...
object DexGenApp extends App with ScoptImplicits with FicusImplicits with EnumerationReader {
  ...
  parser.parse(args, defaultConfig) match {
    ...
    case Some(finalConfig) =>
      ...
      val transfers = Seq(
        massTransfer(endpoint, finalConfig.richAccounts.head, finalConfig.validAccounts, Plix +: tradingAssets.dropRight(2)),
        massTransfer(endpoint, finalConfig.richAccounts.head, finalConfig.fakeAccounts, Plix +: tradingAssets.takeRight(2))
      )
