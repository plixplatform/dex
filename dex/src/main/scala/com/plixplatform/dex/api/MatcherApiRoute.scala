...
import com.plixplatform.dex.AddressActor.GetOrderStatus
import com.plixplatform.dex.AddressDirectory.{Envelope => Env}
...
import com.plixplatform.dex.market.MatcherActor.{ForceStartOrderBook, GetMarkets, GetSnapshotOffsets, MarketData, SnapshotOffsetsResponse}
import com.plixplatform.dex.market.OrderBookActor._
import com.plixplatform.dex.model._
import com.plixplatform.dex.queue.{QueueEvent, QueueEventWithMeta}
import com.plixplatform.dex.settings.{MatcherSettings, OrderRestrictionsSettings, formatValue}
import com.plixplatform.metrics.TimerExt
...
case class MatcherApiRoute(assetPairBuilder: AssetPairBuilder,
  ...
  @Path("/settings/rates")
  @ApiOperation(value = "Asset rates", notes = "Get current rates of assets (asset cost in Plix)", httpMethod = "GET")
  ...
  def upsertRate: Route = (path("settings" / "rates" / AssetPM) & put & withAuth) { a =>
    entity(as[Double]) { rate =>
      withAsset(a) { asset =>
        complete(
          if (asset == Plix) StatusCodes.BadRequest -> wrapMessage("Rate for Plix cannot be changed")
          else {
  ...
  def deleteRate: Route = (path("settings" / "rates" / AssetPM) & delete & withAuth) { a =>
    withAsset(a) { asset =>
      complete(
        if (asset == Plix) StatusCodes.BadRequest -> wrapMessage("Rate for Plix cannot be deleted")
        else {
          val assetStr = AssetPair.assetIdStr(asset)
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "amountAsset", value = "Amount Asset Id in Pair, or 'PLIX'", dataType = "string", paramType = "path"),
      new ApiImplicitParam(name = "priceAsset", value = "Price Asset Id in Pair, or 'PLIX'", dataType = "string", paramType = "path"),
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.plixplatform.transaction.assets.exchange.Order"
      )
    ))
  ...
  @ApiImplicitParams(
    Array(
      ...
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.plixplatform.dex.api.CancelOrderRequest"
      )
    ))
