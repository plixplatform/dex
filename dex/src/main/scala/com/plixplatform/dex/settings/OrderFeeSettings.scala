...
import com.plixplatform.dex.settings.FeeMode.FeeMode
import com.plixplatform.settings.Constants
import com.plixplatform.settings.utils.ConfigSettingsValidator
import com.plixplatform.settings.utils.ConfigSettingsValidator.ErrorsListOr
import com.plixplatform.transaction.Asset
import com.plixplatform.transaction.Asset.{Plix, _}
...
object OrderFeeSettings {
  ...
  val totalPlixAmount: Long = Constants.UnitsInWave * Constants.TotalPlix
  ...
  implicit val orderFeeSettingsReader: ValueReader[OrderFeeSettings] = { (cfg, path) =>
    ...
    def validateDynamicSettings: ErrorsListOr[DynamicSettings] = {
      cfgValidator.validateByPredicate[Long](s"${getPrefixByMode(FeeMode.DYNAMIC)}.base-fee")(
        predicate = fee => 0 < fee && fee <= totalPlixAmount,
        errorMsg = s"required 0 < base fee <= $totalPlixAmount"
      ) map DynamicSettings
    }
    ...
    def validateFixedSettings: ErrorsListOr[FixedSettings] = {
      ...
      val feeValidated = assetValidated match {
        case Valid(Plix) => feeValidator(fee => 0 < fee && fee <= totalPlixAmount, s"required 0 < fee <= $totalPlixAmount")
