...
import com.plixplatform.dex.error._
import com.plixplatform.dex.market.OrderBookActor.MarketStatus
import com.plixplatform.dex.model.MatcherModel.Normalization
import com.plixplatform.dex.settings.OrderFeeSettings._
import com.plixplatform.dex.settings.{AssetType, DeviationsSettings, MatcherSettings, OrderRestrictionsSettings}
import com.plixplatform.dex.smart.MatcherScriptRunner
import com.plixplatform.dex.{RateCache, error}
...
import com.plixplatform.lang.v1.compiler.Terms.{FALSE, TRUE}
...
object OrderValidator extends ScorexLogging {
  ...
  def blockchainAware(blockchain: Blockchain,
    ...
    lazy val validateOrderFeeByTransactionRequirements = orderFeeSettings match {
      case DynamicSettings(baseFee) =>
        ...
        Either.cond(order.matcherFee >= mof, order, error.FeeNotEnough(mof, order.matcherFee, Plix))
      case _ => lift(order)
    }
  ...
  /**
    ...
    * @param rateCache        assets rates (asset cost in Plix)
