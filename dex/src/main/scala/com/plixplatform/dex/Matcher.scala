...
import com.plixplatform.api.http.{ApiRoute, CompositeHttpService => _}
import com.plixplatform.common.utils.{Base58, EitherExt2}
import com.plixplatform.db._
import com.plixplatform.dex.Matcher.Status
import com.plixplatform.dex.api.http.CompositeHttpService
import com.plixplatform.dex.api.{MatcherApiRoute, MatcherApiRouteV1, OrderBookSnapshotHttpCache}
import com.plixplatform.dex.db.{AssetPairsDB, OrderBookSnapshotDB, OrderDB}
import com.plixplatform.dex.error.{ErrorFormatterContext, MatcherError}
import com.plixplatform.dex.history.HistoryRouter
import com.plixplatform.dex.market.OrderBookActor.MarketStatus
import com.plixplatform.dex.market._
import com.plixplatform.dex.model.MatcherModel.{Denormalization, Normalization}
import com.plixplatform.dex.model._
import com.plixplatform.dex.queue._
import com.plixplatform.dex.settings.{MatcherSettings, MatchingRules, RawMatchingRules}
import com.plixplatform.extensions.{Context, Extension}
import com.plixplatform.state.VolumeAndFee
...
import com.plixplatform.utils.{ErrorStartingMatcher, ScorexLogging, forceStopApplication}
...
class Matcher(context: Context) extends Extension with ScorexLogging {
  ...
  private val settings = context.settings.config.as[MatcherSettings]("plix.dex")
  ...
  private val blacklistedAssets: Set[IssuedAsset] = settings.blacklistedAssets
    .map { assetId =>
      AssetPair.extractAssetId(assetId) match {
        case Success(Plix)          => throw new IllegalArgumentException("Can't blacklist the main coin")
  ...
  lazy val matcherApiRoutes: Seq[ApiRoute] = {
    val keyHash = Base58.tryDecode(context.settings.config.getString("plix.rest-api.api-key-hash")).toOption
