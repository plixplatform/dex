...
class AssetPairBuilderSpec extends FreeSpec with Matchers with MockFactory {
  ...
  private val priceAssets       = ConfigFactory.parseString(s"""plix.dex {
       ...
       |  white-list-only     = no
       |  allowed-asset-pairs = [PLIX-${Asset3.id.toString}]
       |}""".stripMargin)

  private val settings   = loadConfig(priceAssets).as[MatcherSettings]("plix.dex")
  ...
  private val pairs = Table(
    ("amount", "price", "result"),
    (PLIX, WUSD.id.toString, Right(())),
    (WUSD.id.toString, PLIX, Left("AssetPairReversed")),
    (WBTC.id.toString, WEUR.id.toString, Left("AssetPairReversed")),
    (WEUR.id.toString, WBTC.id.toString, Right(())),
    (Asset1.id.toString, PLIX, Right(())),
    (PLIX, Asset1.id.toString, Left("AssetPairReversed")),
    ...
    "rejects a pair when" - {
      "blacklist" - {
        "contains asset id" in {
          (blockchain.assetDescription _).when(Asset3).returns(mkAssetDescription())
          builder.validateAssetPair(AssetPair(Asset3, Plix)) should produce("AmountAssetBlacklisted")
        }
      ...
      "pair is not in allowedAssetPairs and whiteListOnly is enabled" in {
        ...
        val assetPair = AssetPair(Plix, WUSD)
