package com.plixplatform.dex.settings

import cats.data.NonEmptyList
import cats.implicits._
import com.typesafe.config.{Config, ConfigFactory}
import com.plixplatform.common.utils.EitherExt2
import com.plixplatform.dex.api.OrderBookSnapshotHttpCache
import com.plixplatform.dex.queue.{KafkaMatcherQueue, LocalMatcherQueue}
import com.plixplatform.dex.settings.OrderFeeSettings.{DynamicSettings, FixedSettings, PercentSettings}
...
import future.com.plixplatform.transaction.assets.exchange.Implicits._
...
class MatcherSettingsSpecification extends FlatSpec with Matchers {
  ...
  def getSettingByConfig(conf: Config): Either[String, MatcherSettings] =
    Try(conf.as[MatcherSettings]("plix.dex")).toEither.leftMap(_.getMessage)
  ...
  "MatcherSettings" should "read values" in {
    ...
    val settings = config.as[MatcherSettings]("plix.dex")
    ...
    settings.journalDataDir should be("/plix/matcher/journal")
    settings.snapshotsDataDir should be("/plix/matcher/snapshots")
    ...
    settings.priceAssets should be(Seq("PLIX", "8LQW8f7P5d5PZM7GtZEBgaqRPGSzS3DfPuiXrURJ4AJS", "DHgwrRvVyqJsepd32YbBqUeDH4GJ1N984X8QoekjgH8J"))
  ...
  "OrderFeeSettings in MatcherSettings" should "be validated" in {
    ...
    settingsInvalidFeeInDynamicMode shouldBe Left(
      s"Invalid setting order-fee.dynamic.base-fee value: -350000 (required 0 < base fee <= ${OrderFeeSettings.totalPlixAmount})"
    )
  }
  ...
  "Allowed asset pairs in MatcherSettings" should "be validated" in {
    ...
    val nonEmptyCorrect =
      """allowed-asset-pairs = [
        | "PLIX-BTC",
        | "PLIX-ETH",
        | "8LQW8f7P5d5PZM7GtZEBgaqRPGSzS3DfPuiXrURJ4AJS-PLIX"
        |]
      """.stripMargin

    getSettingByConfig(configStr(incorrectAssetsCount)) should produce(
      "Invalid setting allowed-asset-pairs value: PLIX-BTC-ETH (incorrect assets count, expected 2 but got 3), ETH (incorrect assets count, expected 2 but got 1)"
    )

    getSettingByConfig(configStr(incorrectAssets)) should produce(
      "Invalid setting allowed-asset-pairs value: PLIX-;;; (requirement failed: Wrong char ';' in Base58 string ';;;')"
    )
    ...
    getSettingByConfig(configStr(nonEmptyCorrect)).explicitGet().allowedAssetPairs shouldBe
      Set(
        AssetPair.createAssetPair("PLIX", "BTC").get,
        AssetPair.createAssetPair("PLIX", "ETH").get,
        AssetPair.createAssetPair("8LQW8f7P5d5PZM7GtZEBgaqRPGSzS3DfPuiXrURJ4AJS", "PLIX").get
      )
  }
    ...
    withClue("incorrect pair and step amount") {
      getSettingByConfig(configStr(incorrectPairAndStepAmount)) should produce(
        "Invalid setting order-restrictions value: Can't parse asset pair 'ETH-;;;', " +
          "Invalid setting order-restrictions.PLIX-BTC.step-amount value: -0.013 (required 0 < value)"
      )
    }

    withClue("incorrect min and max") {
      getSettingByConfig(configStr(incorrectMinAndMax)) should produce(
        "Required order-restrictions.PLIX-BTC.min-price < order-restrictions.PLIX-BTC.max-price")
    }
  }
  ...
  "Matching rules" should "be validated" in {
    ...
    val nonEmptyCorrect =
      """matching-rules = {
        |  "PLIX-8LQW8f7P5d5PZM7GtZEBgaqRPGSzS3DfPuiXrURJ4AJS": [
        |    {
        |      start-offset = 100
        |      tick-size    = 0.002
        |    },
    ...
    withClue("nonempty correct") {
      getSettingByConfig(configStr(nonEmptyCorrect)).explicitGet().matchingRules shouldBe Map(
        AssetPair.fromString("PLIX-8LQW8f7P5d5PZM7GtZEBgaqRPGSzS3DfPuiXrURJ4AJS").get ->
