...
import com.plixplatform.dex.model.OrderBook.{LastTrade, Level, SideSnapshot, Snapshot}
import com.plixplatform.dex.settings.MatchingRules
...
import com.plixplatform.{NTPTime, NoShrink}
...
class OrderBookSpec extends FreeSpec with PropertyChecks with Matchers with MatcherTestData with NTPTime with NoShrink {
  val pair: AssetPair = AssetPair(Plix, mkAssetId("BTC"))
  ...
  "buy small amount of pricey asset" in {
    val p = AssetPair(IssuedAsset(ByteStr("PLIX".getBytes)), IssuedAsset(ByteStr("USD".getBytes)))
