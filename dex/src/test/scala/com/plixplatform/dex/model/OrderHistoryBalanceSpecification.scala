...
import com.plixplatform.dex.{AddressActor, MatcherTestData}
...
class OrderHistoryBalanceSpecification
  ...
  private val PlixBtc = AssetPair(Plix, mkAssetId("BTC"))
  ...
  property("New buy order added") {
    ...
    withClue("reserved assets") {
      ...
      openVolume(ord.senderPublicKey, Plix) shouldBe ord.matcherFee
    }
  ...
  property("New buy PLIX order added") {
    val ord = buy(PlixBtc, 1000, 0.008, matcherFee = Some(3000))
    val lo  = LimitOrder(ord)
    ...
    withClue("reserved assets considering amount of received PLIX") {
      openVolume(ord.senderPublicKey, PlixBtc.amountAsset) shouldBe 2000L
      openVolume(ord.senderPublicKey, PlixBtc.priceAsset) shouldBe 8L
    }

    activeOrderIds(ord.senderPublicKey) shouldBe Seq(ord.id())
  }

  property("New sell PLIX order added") {
    val ord = sell(PlixBtc, 10000, 0.0008)
    val lo  = LimitOrder(ord)

    oh.process(OrderAdded(lo, ntpTime.getTimestamp()))
    orderStatus(ord.id()) shouldBe OrderStatus.Accepted
    openVolume(ord.senderPublicKey, PlixBtc.amountAsset) shouldBe 10000L + ord.matcherFee
    openVolume(ord.senderPublicKey, PlixBtc.priceAsset) shouldBe 0L

    activeOrderIds(ord.senderPublicKey) shouldBe Seq(ord.id())
  }

  property("Should not reserve fee, if seller receives more PLIX than total fee in sell order") {
    val pair = AssetPair(mkAssetId("BTC"), Plix)
  ...
  property("Should not reserve fee, if buyer receives more PLIX than total fee in buy order") {
    val ord = buy(PlixBtc, 100000, 0.0007, matcherFee = Some(1000L))

    oh.process(OrderAdded(LimitOrder(ord), ntpTime.getTimestamp()))
    orderStatus(ord.id()) shouldBe OrderStatus.Accepted

    openVolume(ord.senderPublicKey, PlixBtc.amountAsset) shouldBe 0L
  }

  property("Two sell orders added") {
    val pk   = KeyPair("private".getBytes("utf-8"))
    val ord1 = sell(PlixBtc, 10000, 0.0005, Some(pk), matcherFee = Some(30000L), ts = Some(System.currentTimeMillis()))
    val ord2 = sell(PlixBtc, 16000, 0.0008, Some(pk), matcherFee = Some(30000L), ts = Some(System.currentTimeMillis() + 1))
    ...
    withClue("correction was used to reserve assets") {
      openVolume(ord1.senderPublicKey, PlixBtc.amountAsset) shouldBe ord1.amount + ord1.matcherFee + ord2.amount + ord2.matcherFee
      openVolume(ord1.senderPublicKey, PlixBtc.priceAsset) shouldBe 0L
    }
    ...
    withClue("orders list") {
      ...
      activeOrderIdsByPair(ord1.senderPublicKey, PlixBtc) shouldBe expected
      allOrderIdsByPair(ord1.senderPublicKey, PlixBtc) shouldBe expected
    }
  }

  property("Buy PLIX order filled exactly") {
    val counter   = buy(PlixBtc, 100000, 0.0008, matcherFee = Some(2000L))
    val submitted = sell(PlixBtc, 100000, 0.0007, matcherFee = Some(1000L))
    ...
    withClue(s"has no reserved assets, counter.senderPublicKey: ${counter.senderPublicKey}, counter.order.id=${counter.idStr()}") {
      openVolume(counter.senderPublicKey, PlixBtc.amountAsset) shouldBe 0L
      openVolume(counter.senderPublicKey, PlixBtc.priceAsset) shouldBe 0L
    }

    withClue(s"has no reserved assets, submitted.senderPublicKey: ${submitted.senderPublicKey}, submitted.order.id=${submitted.idStr()}") {
      openVolume(submitted.senderPublicKey, PlixBtc.amountAsset) shouldBe 0L
      openVolume(submitted.senderPublicKey, PlixBtc.priceAsset) shouldBe 0L
    }
    ...
    withClue(s"orders list of counter owner ${counter.sender}") {
      ...
      activeOrderIdsByPair(counter.senderPublicKey, PlixBtc) shouldBe empty
      allOrderIdsByPair(counter.senderPublicKey, PlixBtc) shouldBe Seq(counter.id())
    }
    ...
    withClue(s"orders list of submitted owner ${submitted.sender}") {
      ...
      activeOrderIdsByPair(submitted.senderPublicKey, PlixBtc) shouldBe empty
      allOrderIdsByPair(submitted.senderPublicKey, PlixBtc) shouldBe Seq(submitted.id())
    }
  }

  property("Buy PLIX order filled with remainder") {
    val counter   = sell(PlixBtc, 840340L, 0.00000238, matcherFee = Some(300000L))
    val submitted = buy(PlixBtc, 425532L, 0.00000238, matcherFee = Some(300000L))
    ...
    withClue("orders list of counter owner") {
      ...
      activeOrderIdsByPair(counter.senderPublicKey, PlixBtc) shouldBe expected
      allOrderIdsByPair(counter.senderPublicKey, PlixBtc) shouldBe expected
    }
  ...
  property("Sell PLIX order - filled, buy order - partial") {
    val counter   = sell(PlixBtc, 100000000, 0.0008, matcherFee = Some(2000L))
    val submitted = buy(PlixBtc, 120000000, 0.00085, matcherFee = Some(1000L))
...
//                 OrderInfo.safeSum(LimitOrder.getPartialFee(submitted.matcherFee, submitted.amount, submitted.amount - counter.amount), -20000000L))
      openVolume(submitted.senderPublicKey, PlixBtc.priceAsset) shouldBe (BigDecimal(0.00085) * 20000000L).toLong
    }
    ...
    withClue("orders list of submitted owner") {
      ...
      activeOrderIdsByPair(submitted.senderPublicKey, PlixBtc) shouldBe expected
      allOrderIdsByPair(submitted.senderPublicKey, PlixBtc) shouldBe expected
    }
  }

  property("Buy PLIX order - filled with 2 steps, sell order - partial") {
    val counter    = buy(PlixBtc, 100000000, 0.0008, matcherFee = Some(300001L))
    val submitted1 = sell(PlixBtc, 50000000, 0.00075, matcherFee = Some(300001L))
    val submitted2 = sell(PlixBtc, 80000000, 0.0008, matcherFee = Some(300001L))
    ...
    openVolume(submitted1.senderPublicKey, PlixBtc.priceAsset) shouldBe 0L
    openVolume(submitted1.senderPublicKey, PlixBtc.amountAsset) shouldBe 0L
    ...
    withClue(s"account checks, ord3.senderPublicKey: ${submitted2.senderPublicKey}, ord3.order.id=${submitted2.idStr()}") {
      openVolume(submitted2.senderPublicKey, PlixBtc.priceAsset) shouldBe 0L
  ...
  property("WCT/BTC: sell - filled partially, buy - filled") {
    ...
    withClue(s"account checks, counter.senderPublicKey: ${counter.senderPublicKey}, counter.order.id=${counter.id()}") {
      ...
      openVolume(counter.senderPublicKey, Plix) shouldBe counter.matcherFee - LimitOrder.partialFee(counter.matcherFee,
                                                                                                     counter.amount,
                                                                                                     exec.executedAmount)
    }
    ...
    withClue(s"account checks, submitted.senderPublicKey: ${submitted.senderPublicKey}, submitted.order.id=${submitted.id()}") {
      ...
      openVolume(submitted.senderPublicKey, Plix) shouldBe 0L
    }
  }
  ...
  property("Sell ETH twice (filled, partial), buy PLIX order - filled") {
    val pair      = AssetPair(mkAssetId("ETH"), Plix)
  ...
  property("Total execution of two counter orders and the one submitted") {
    val pair = AssetPair(mkAssetId("Alice"), Plix)
  ...
  property("Partially with own order") {
    val pk        = KeyPair("private".getBytes("utf-8"))
    val counter   = buy(PlixBtc, 100000000, 0.0008, Some(pk), Some(300000L))
    val submitted = sell(PlixBtc, 210000000, 0.00079, Some(pk), Some(300000L))
    ...
    openVolume(pk, PlixBtc.amountAsset) shouldBe 110157143L
    openVolume(pk, PlixBtc.priceAsset) shouldBe 0L

    withClue("orders list") {
      activeOrderIds(pk) shouldBe Seq(submitted.id())
      allOrderIds(pk) shouldBe Seq(submitted.id(), counter.id())

      activeOrderIdsByPair(pk, PlixBtc) shouldBe Seq(submitted.id())
      allOrderIdsByPair(pk, PlixBtc) shouldBe Seq(submitted.id(), counter.id())
    }
  }
  ...
  property("Cancel partially executed order") {
    val counter   = sell(PlixBtc, 2100000000, 0.0008, matcherFee = Some(300000L))
    val submitted = buy(PlixBtc, 1000000000, 0.00081, matcherFee = Some(300000L))
    ...
    withClue("orders list of counter owner") {
      ...
      activeOrderIdsByPair(addr, PlixBtc) shouldBe empty
      allOrderIdsByPair(addr, PlixBtc) shouldBe Seq(counter.id())
    }
  ...
  property("Sorting by status then timestamp") {
    val pk   = KeyPair("private".getBytes("utf-8"))
    val ord1 = buy(PlixBtc, 110000000, 0.0008, Some(pk), Some(300000L), Some(1L)) // Filled
    val ord2 = buy(PlixBtc, 120000000, 0.0006, Some(pk), Some(300000L), Some(2L)) // Accepted
    val ord3 = buy(PlixBtc, 130000000, 0.0005, Some(pk), Some(300000L), Some(3L)) // Canceled
    val ord4 = sell(PlixBtc, 2100000000, 0.00079, Some(pk), Some(300000L), Some(4L)) // Partial
    val ord5 = buy(PlixBtc, 130000000, 0.0004, Some(pk), Some(300000L), Some(45)) // Accepted
    ...
    withClue("orders list") {
      ...
      activeOrderIdsByPair(addr, PlixBtc) shouldBe activeOrders
      allOrderIdsByPair(addr, PlixBtc) shouldBe allOrders
    }
  }
  ...
  property("History with more than max limit") {
    ...
    val origOrders = (0 until matcherSettings.maxOrdersPerRequest).map { i =>
      val o = buy(PlixBtc, 100000000, 0.0008 + 0.00001 * i, Some(pk), Some(300000L), Some(100L + i))
    ...
    val newOrder = buy(PlixBtc, 100000000, 0.001, Some(pk), Some(300000L), Some(1L))
  ...
  property("History by pair - added orders more than history by pair limit (200 active)") {
    ...
    withClue("pair") {
      ...
      allOrderIdsByPair(pk, PlixBtc) shouldBe expectedIds
      activeOrderIdsByPair(pk, PlixBtc) shouldBe expectedIds
    }
  }
  ...
  property("History by pair - added and canceled orders both more than history by pair limit (200 active, 10 canceled)") {
    ...
    withClue("pair") {
      val expectedIds = allOrders.takeRight(MaxElements).map(_.id()).reverse
      val pair1Ids    = allOrderIdsByPair(pk, PlixBtc)
      pair1Ids shouldBe expectedIds
      activeOrderIdsByPair(pk, PlixBtc) shouldBe expectedActiveOrderIds
    }
  }
  ...
  property("History by pair contains more elements than in common") {
    val pk    = KeyPair("private".getBytes("utf-8"))
    val pair1 = PlixBtc
    val pair2 = AssetPair(Plix, mkAssetId("ETH"))
