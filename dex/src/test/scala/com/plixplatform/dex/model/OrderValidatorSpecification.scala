...
import com.plixplatform.dex.error.ErrorFormatterContext
...
import com.plixplatform.dex.market.OrderBookActor.MarketStatus
import com.plixplatform.dex.model.MatcherModel.{Denormalization, Normalization}
import com.plixplatform.dex.model.OrderValidator.Result
import com.plixplatform.dex.settings.OrderFeeSettings.{DynamicSettings, FixedSettings, OrderFeeSettings, PercentSettings}
import com.plixplatform.dex.settings.{AssetType, DeviationsSettings, OrderRestrictionsSettings}
import com.plixplatform.dex.{MatcherTestData, RateCache}
import com.plixplatform.settings.Constants
import com.plixplatform.state.diffs.produce
import com.plixplatform.state.{AssetDescription, Blockchain, LeaseBalance, Portfolio}
...
import com.plixplatform.utils.randomBytes
import com.plixplatform.{NoShrink, TestTime, WithDB}
...
class OrderValidatorSpecification
  ...
  private val pairPlixBtc       = AssetPair(Plix, wbtc)
  ...
  "OrderValidator" should {
    "allow buying PLIX for BTC without balance for order fee" in asa() { v =>
      v shouldBe 'right
    }
    ...
    "reject new order" when {
      ...
      "matcherFee is too small (dynamic mode)" in forAll(orderV3WithPredefinedFeeAssetGenerator(Some(Plix))) {
      ...
      "matcherFee is insufficient in case of scripted account or asset" in forAll(orderWithoutPlixInPairAndWithFeeSettingsGenerator) {
      ...
      "sell order's price is out of deviation bounds (market aware)" in {
        val fixedPlixFeeSettings = DynamicSettings(300000L)
        ...
        val assetPair2MarketStatus = new ConcurrentHashMap[AssetPair, MarketStatus]
        assetPair2MarketStatus.put(pairPlixBtc, MarketStatus(None, Some(bestBid), Some(bestAsk)))
        val nonEmptyMarketStatus = assetPair2MarketStatus.get(pairPlixBtc)
        ...
        val orderValidator = OrderValidator.marketAware(fixedPlixFeeSettings, deviationSettings, Option(nonEmptyMarketStatus), rateCache) _
      ...
      "order's fee is out of deviation bounds (market aware)" in {
        ...
        val assetPair2MarketStatus = new ConcurrentHashMap[AssetPair, MarketStatus]
        assetPair2MarketStatus.put(pairPlixBtc, MarketStatus(None, None, Some(bestAsk)))
        val nonEmptyMarketStatus = assetPair2MarketStatus.get(pairPlixBtc)
      ...
      forAll(orderV3WithFeeSettingsGenerator) {
        case (order, orderFeeSettings) =>
          ...
          orderFeeSettings match {
            ...
            case _ =>
              // case _: FixedPlixSettings => it's impossible to set script for Plix
  ...
  private def newBuyOrder: Order =
    buy(pair = pairPlixBtc, amount = 100 * Constants.UnitsInWave, price = 0.0022, matcherFee = Some((0.003 * Constants.UnitsInWave).toLong))
